...
def get_context(context):
	PagesList=frappe.db.get_all('Pages',fields=['name1','published','route','description','image','page_type'], filters={'published':1,'page_type':'Gss Articles' } )
	context.PagesList=PagesList 
	EventsList=frappe.db.sql('''select * from `tabEvents` where published=1 
		and start_date>%(now)s and event_type=%(event_type)s order by start_date'''
		,{'now':getdate(nowdate()),'event_type':'GSS Events'},as_dict=1)	
