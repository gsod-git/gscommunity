...
frappe.ui.form.on('GSOD Promotions', {
	refresh: function(frm) {
        ...
        if(!cur_frm.doc.__islocal && cur_frm.doc.status=='Waiting for approval'){
            cur_frm.add_custom_button(__('Approve'), function() {
                frappe.call({
                    method:"gscommunity.gscommunity.doctype.gsod_promotions.gsod_promotions.change_status",
                    args:{
...
frappe.ui.form.on("GSOD Promotions", "sponsor", function(frm, cdt, cdn) {
    if (frm.doc.sponsor) {
        frappe.call({
            method: "gscommunity.gscommunity.doctype.gsod_promotions.gsod_promotions.get_duration",
            args: {
            ...
            callback: function(r) {
                ...
                if(r){
                    ...
                    frm.set_value("max_no_of_views", r.message.impression);
                }
            }
        })
    }
